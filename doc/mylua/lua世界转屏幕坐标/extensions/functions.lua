...
function table.clone(object)
    ...
    local function _copy(object)
        ...
        for key, value in TPairs(object) do
...
function table.merge(dest, src) for k, v in TPairs(src) do dest[k] = v end end
...
--     for k, v in TPairs(hashtable) do
...
function table.nums(t)
    local count = 0
    for _, _ in TPairs(t) do count = count + 1 end
    return count
end
...
function Serialize(obj)
    ...
    elseif t == "table" then
        lua = lua .. "{\n"
        for k, v in TPairs(obj) do
            lua = lua .. "[" .. Serialize(k) .. "]=" .. Serialize(v) .. ",\n"
        end
        local metatable = getmetatable(obj)
        if metatable ~= nil and type(metatable.__index) == "table" then
            for k, v in TPairs(metatable.__index) do
                lua = lua .. "[" .. Serialize(k) .. "]=" .. Serialize(v) ..
                          ",\n"
            end
        end
